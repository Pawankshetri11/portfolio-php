                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('experiences.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="company" id="company" value="{{ old('company') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @error('company') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="start_date" class="block text-sm font-medium text-gray-700">Start Date</label>
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @error('start_date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="end_date" class="block text-sm font-medium text-gray-700">End Date (leave empty for present)</label>
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @error('end_date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="location" class="block text-sm font-medium text-gray-700">Location</label>
                            <input type="text" name="location" id="location" value="{{ old('location') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="e.g., Remote, On-site">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="display_type" id="display_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="responsibilities" {{ old('display_type', 'responsibilities') == 'responsibilities' ? 'selected' : '' }}>Key Responsibilities (List)</option>
                                <option value="description" {{ old('display_type') == 'description' ? 'selected' : '' }}>Description (Paragraph)</option>
                            </select>
                            @error('display_type') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4" id="responsibilities-group">
                            <label for="responsibilities" class="block text-sm font-medium text-gray-700">Key Responsibilities (one per line)</label>
                            <textarea name="responsibilities" id="responsibilities" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="Enter each responsibility on a new line">{{ old('responsibilities') }}</textarea>
                            @error('responsibilities') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4" id="description-group" style="display: none;">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="technologies" id="technologies" value="{{ old('technologies') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="e.g., PHP, Laravel, JavaScript">
